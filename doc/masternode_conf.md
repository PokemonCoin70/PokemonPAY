 ...
 * Windows: %APPDATA%\Pokemon\
 * Mac OS: ~/Library/Application Support/Pokemon/
 * Unix/Linux: ~/.pokemon/
...
* the collateral for `mn1` is output `0` of transaction http://test.explorer.pokemon.fr/tx/2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c and it has amount of 1000 POKEMON
...
When using remote masternode setup, you **must** remove `masternode=1` from local `pokemon.conf` file if you had it there in previous versions. This option should be used only to start local Hot masternode now.
