...
int nAnonymizePokemonAmount = 1000;
...
const char * const BITCOIN_CONF_FILENAME = "pokemon.conf";
const char * const BITCOIN_PID_FILENAME = "pokemond.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "pokemon" is a composite category enabling all Pokemon-related debug output
            if(ptrCategory->count(string("pokemon"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Pokemon
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Pokemon
    // Mac: ~/Library/Application Support/Pokemon
    // Unix: ~/.pokemon
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Pokemon";
#else
    fs::path pathRet;
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty pokemon.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override pokemon.conf
