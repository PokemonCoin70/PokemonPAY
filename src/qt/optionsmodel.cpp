...
void OptionsModel::Init(bool resetSettings)
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::POKEMON);
    ...
    if (!settings.contains("nAnonymizePokemonAmount")) {
        // for migration from old settings
        if (!settings.contains("nAnonymizeDarkcoinAmount"))
            settings.setValue("nAnonymizePokemonAmount", 1000);
        else
            settings.setValue("nAnonymizePokemonAmount", settings.value("nAnonymizeDarkcoinAmount").toInt());
    }
    if (!SoftSetArg("-anonymizepokemonamount", settings.value("nAnonymizePokemonAmount").toString().toStdString()))
        addOverriddenOption("-anonymizepokemonamount");
    nAnonymizePokemonAmount = settings.value("nAnonymizePokemonAmount").toInt();
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in pokemon.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizePokemonAmount:
            return settings.value("nAnonymizePokemonAmount");
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case PrivateSendRounds:
            ...
            break;
        case AnonymizePokemonAmount:
            if (settings.value("nAnonymizePokemonAmount") != value)
            {
                nAnonymizePokemonAmount = value.toInt();
                settings.setValue("nAnonymizePokemonAmount", nAnonymizePokemonAmount);
                Q_EMIT anonymizePokemonAmountChanged();
            }
            break;
        case ShowMasternodesTab:
