 ...
 *
 *   // network
 *   CPokemonNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.pokemon.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CPokemonNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CPokemonNetwork(UniValue objIn)
    {
...
// // can be under: PokemonNetwork
// //   -- signature requirements : Key1(User)
// class CPokemonNetworkVariable : public CGovernanceObject
// {
// private:
