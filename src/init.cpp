...
#include "config/pokemon-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, mempoolrej, net, proxy, prune, http, libevent, tor, zmq, "
                             "pokemon (or specifically: privatesend, instantsend, masternode, keepass, mnpayments, mngovernance)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Pokemon specific functionality (Masternodes, PrivateSend, InstantSend, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-anonymizepokemonamount=<n>", strprintf(_("Keep N POKEMON anonymized (default: %u)"), nAnonymizePokemonAmount));
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i The Pokemon Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Pokemon is running in a usable environment with all
...
void InitParameterInteraction()
{
    ...
    if (GetArg("-liquidityprovider", 0) > 0) {
        ...
        mapArgs["-anonymizepokemonamount"] = "999999";
        LogPrintf("AppInit2 : parameter interaction: -liquidityprovider=%d -> setting -anonymizepokemonamount=999999\n", nLiqProvTmp);
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Pokemon Core is shutting down."));
...
#endif
    // Make sure only a single Pokemon process is using the data directory.
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Pokemon Core is probably already running."), strDataDir));
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Pokemon Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Pokemon Core to complete") << "\n";
    ...
    nAnonymizePokemonAmount = GetArg("-anonymizepokemonamount", nAnonymizePokemonAmount);
    nAnonymizePokemonAmount = std::min(std::max(nAnonymizePokemonAmount, 2), 999999);
    ...
    LogPrintf("Anonymize Pokemon Amount %d\n", nAnonymizePokemonAmount);
    ...
    // ********************************************************* Step 11c: update block tip in Pokemon modules
    ...
    // ********************************************************* Step 11d: start pokemon-privatesend thread
