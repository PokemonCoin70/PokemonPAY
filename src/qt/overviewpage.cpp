...
public:
    TxViewDelegate(const PlatformStyle *platformStyle):
        QAbstractItemDelegate(), unit(BitcoinUnits::POKEMON),
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    if(model && model->getOptionsModel())
    {
        ...
        connect(model->getOptionsModel(), SIGNAL(anonymizePokemonAmountChanged()), this, SLOT(updatePrivateSendProgress()));
    ...
    // update the display unit, to not use the default ("POKEMON")
    updateDisplayUnit();
}
...
void OverviewPage::updatePrivateSendProgress()
{
    ...
    QString strAnonymizePokemonAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizePokemonAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizePokemonAmount = strAnonymizePokemonAmount.remove(strAnonymizePokemonAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePokemonAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizePokemonAmount*COIN) nMaxToAnonymize = nAnonymizePokemonAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizePokemonAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizePokemonAmount));
        strAnonymizePokemonAmount = strAnonymizePokemonAmount.remove(strAnonymizePokemonAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePokemonAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    } else {
